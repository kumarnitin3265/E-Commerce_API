...
1) run "npm init" command on terminal in this projects directory then npm install command for nodemon install.
2) start the server using node app.js /or also start with npm start
3) Open postman
4) Make a GET request on localhost:3000/products
5) The products should be visible

STEPS TO CREATE A NEW PRODUCT: 
1) start the server using node app.js /or also start with npm start
2) Open postman
